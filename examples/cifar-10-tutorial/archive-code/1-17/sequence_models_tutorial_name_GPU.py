...
lstm.cuda(gpu_id)
inputs = [autograd.Variable(torch.randn((1, 3)).cuda(gpu_id))
          for _ in range(5)]  # make a sequence of length 5

# initialize the hidden state.
hidden = (autograd.Variable(torch.randn(1, 1, 3).cuda(gpu_id)),
          autograd.Variable(torch.randn((1, 1, 3)).cuda(gpu_id)))
for i in inputs:
...
inputs = torch.cat(inputs).view(len(inputs), 1, -1).cuda(gpu_id)
hidden = (autograd.Variable(torch.randn(1, 1, 3).cuda(gpu_id)), autograd.Variable(
    torch.randn((1, 1, 3)).cuda(gpu_id)))  # clean out hidden state
