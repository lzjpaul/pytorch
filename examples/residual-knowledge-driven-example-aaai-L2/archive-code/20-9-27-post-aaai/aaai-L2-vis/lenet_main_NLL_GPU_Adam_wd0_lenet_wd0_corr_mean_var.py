...
def train(epoch, net, data_train_loader, vis_data_train_loader, optimizer, criterion, logger, res_regularizer_diff_dim_instance, model_name, firstepochs, labelnum,\
    ...
    if epoch == 1 or ((epoch+1) % int(max_epoch/4)) == 0:
        ...
        for i in range(len(feature_dim_vec)-1):
            np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/lenet_verify_correlation_avg_matrix_wd0_' + str(i) + '_' + str(model_name) + '_' + str(epoch), verify_correlation_avg[i], fmt = '%6f', delimiter=",") #modify here
            np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/lenet_verify_correlation_var_avg_matrix_wd0_' + str(i) + '_' + str(model_name) + '_' + str(epoch), verify_correlation_var_avg[i], fmt = '%6f', delimiter=",") #modify here
