...
def training(model, train_loader, vis_train_loader, Epochs, test_loader, device, optimizer, criterion, model_name, prior_beta, reg_lambda, momentum_mu, weightdecay, firstepochs, labelnum, regmethod, lasso_strength, max_val):
    ...
    for epoch in range(Epochs):
        ...
        if epoch <= 10 or ((epoch+1) % int(Epochs/4)) == 0:
            ...
            for i in range(len(feature_dim_vec)-1):
                np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/autoencoder_verify_correlation_avg_matrix_wd0_' + str(i) + '_' + str(model_name) + '_' + str(epoch), verify_correlation_avg[i], fmt = '%6f', delimiter=",") #modify here
                np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/autoencoder_verify_correlation_var_avg_matrix_wd0_' + str(i) + '_' + str(model_name) + '_' + str(epoch), verify_correlation_var_avg[i], fmt = '%6f', delimiter=",") #modify here
