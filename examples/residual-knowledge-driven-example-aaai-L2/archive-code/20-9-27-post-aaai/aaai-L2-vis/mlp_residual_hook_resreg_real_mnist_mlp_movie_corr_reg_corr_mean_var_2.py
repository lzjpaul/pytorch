...
def train_validate_test_resmlp_model(model_name, model, gpu_id, train_loader, vis_train_loader, test_loader, criterion, optimizer, reg_method, prior_beta, reg_lambda, momentum_mu, blocks, hidden_dim, weightdecay, firstepochs, labelnum, lasso_strength, max_val, max_epoch=25):
    ...
    for epoch in range(max_epoch):
        ...
        # 20-8-20: Iterate over train data to get correlation
        if epoch == 0 or ((epoch+1) % int(max_epoch/4)) == 0:
            ...
            if labelnum > 1:
                np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/mimic-iii_verify_correlation_avg_matrix' + '_' + str(res_regularizer_instance.reg_lambda) + '_' + str(res_regularizer_instance.prior_beta) + '_' + str(model_name) + str(epoch), verify_correlation_avg, fmt = '%6f', delimiter=",") #modify here
                np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/mimic-iii_verify_correlation_var_avg_matrix' + '_' + str(res_regularizer_instance.reg_lambda) + '_' + str(res_regularizer_instance.prior_beta) + '_' + str(model_name) + str(epoch), verify_correlation_var_avg, fmt = '%6f', delimiter=",") #modify here
            else:
                np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/movie_review_verify_correlation_avg_matrix' + '_' + str(res_regularizer_instance.reg_lambda) + '_' + str(res_regularizer_instance.prior_beta) + '_' + str(model_name) + str(epoch), verify_correlation_avg, fmt = '%6f', delimiter=",") #modify here
                np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/movie_review_verify_correlation_var_avg_matrix' + '_' + str(res_regularizer_instance.reg_lambda) + '_' + str(res_regularizer_instance.prior_beta) + '_' + str(model_name) + str(epoch), verify_correlation_var_avg, fmt = '%6f', delimiter=",") #modify here
...
def train_validate_test_resmlp_model_MNIST(model_name, model, gpu_id, train_loader, vis_train_loader, test_loader, criterion, optimizer, reg_method, prior_beta, reg_lambda, momentum_mu, blocks, hidden_dim, weightdecay, firstepochs, labelnum, lasso_strength, max_val, max_epoch=25):
    ...
    for epoch in range(max_epoch):
        ...
        if epoch == 0 or ((epoch+1) % int(max_epoch/4)) == 0:
            ...
            verify_correlation_avg = np.zeros((hidden_dim, hidden_dim))
            with torch.no_grad():
                for batch_idx, (data, target) in enumerate(vis_train_loader):
                    data = data.reshape((data.shape[0],-1))
                    data, target = data.cuda(gpu_id), target.cuda(gpu_id)
                    features.clear()
                    output = model(data)
            ...
            # verify_correlation_var_avg = verify_correlation_var_avg / float(batch_idx + 1) 
            verify_correlation_var_avg = verify_correlation_var_avg / float(non_nan_count)
            np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/mnist_verify_correlation_avg_matrix' + '_' + str(res_regularizer_instance.reg_lambda) + '_' + str(res_regularizer_instance.prior_beta) + '_' + str(model_name) + str(epoch), verify_correlation_avg, fmt = '%6f', delimiter=",") #modify here
            np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/mnist_verify_correlation_var_avg_matrix' + '_' + str(res_regularizer_instance.reg_lambda) + '_' + str(res_regularizer_instance.prior_beta) + '_' + str(model_name) + str(epoch), verify_correlation_var_avg, fmt = '%6f', delimiter=",") #modify here
...
if __name__ == '__main__':
    ...
    ########## using for
    weightdecay_list = [0.01]
    reglambda_list = [1e-4]
    priorbeta_list = [0.001, 0.01]
