...
def train(train_loader, vis_train_loader, model, criterion, optimizer, epoch, model_name, prior_beta, reg_lambda, momentum_mu, weightdecay, firstepochs, labelnum, \
    ...
    if epoch == 0 or ((epoch+1) % int(max_epoch/4)) == 0:
        ...
        with torch.no_grad():
            for data_i, (input, target) in enumerate(vis_train_loader):
                ...
                for i in range(blocks):
                    ...
                    ### only this layer ...
                    correlation_added = np.corrcoef(feature_matrix, second_feature_matrix, rowvar=False)[feature_dim:, 0:feature_dim]  # only this layer
        ...
        for i in range(blocks):
            np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/vgg_verify_correlation_avg_matrix_' + '_' + str(res_regularizer_instance.reg_lambda) + '_' + str(res_regularizer_instance.prior_beta) + '_' + str(i) + '_' + str(model_name) + '_' + str(epoch), verify_correlation_avg[i], fmt = '%6f', delimiter=",") #modify here
            np.savetxt('/hdd2/zhaojing/res-regularization/20-8-17/saved_corr_mean_var/vgg_verify_correlation_var_avg_matrix_' + '_' + str(res_regularizer_instance.reg_lambda) + '_' + str(res_regularizer_instance.prior_beta) + '_' + str(i) + '_' + str(model_name) + '_' + str(epoch), verify_correlation_var_avg[i], fmt = '%6f', delimiter=",") #modify here
