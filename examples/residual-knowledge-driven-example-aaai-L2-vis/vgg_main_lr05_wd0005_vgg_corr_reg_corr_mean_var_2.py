...
def train(train_loader, vis_train_loader, model, criterion, optimizer, epoch, model_name, prior_beta, reg_lambda, momentum_mu, weightdecay, firstepochs, labelnum, \
    ...
    if epoch == 0 or ((epoch+1) % int(max_epoch/4)) == 0:
        ...
        for i in range(blocks):
            np.savetxt('vgg_verify_correlation_avg_matrix_' + str(i) + '_' + str(model_name) + '_' + str(epoch), verify_correlation_avg[i], fmt = '%6f', delimiter=",") #modify here
            np.savetxt('vgg_verify_correlation_var_avg_matrix_' + str(i) + '_' + str(model_name) + '_' + str(epoch), verify_correlation_var_avg[i], fmt = '%6f', delimiter=",") #modify here
