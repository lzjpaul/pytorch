...
def train_validate_test_resmlp_model(model_name, model, gpu_id, train_loader, vis_train_loader, test_loader, criterion, optimizer, reg_method, prior_beta, reg_lambda, momentum_mu, blocks, hidden_dim, weightdecay, firstepochs, labelnum, max_epoch=25):
    logger = logging.getLogger('res_reg')
    ...
    for epoch in range(max_epoch):
        ...
        for batch_idx, data_iter in enumerate(train_loader, 0):
            ...
            # print ("train features length: ", len(features))
        ...
        if epoch < 6:
            ...
            with torch.no_grad():
                for batch_idx, data_iter in enumerate(vis_train_loader, 0):
                    ...
                    features.clear()
                    outputs = model(data_x)
                    # print ("avg features length: ", len(features))
                    print ('features[0].data.cpu().numpy() shape: ', features[0].data.cpu().numpy().shape)
                    print ('features[1].data.cpu().numpy() shape: ', features[1].data.cpu().numpy().shape)
            ...
            with torch.no_grad():
                for batch_idx, data_iter in enumerate(vis_train_loader, 0):
                    ...
                    features.clear()
                    outputs = model(data_x)
                    # print ("var features length: ", len(features))
...
def train_validate_test_resmlp_model_MNIST(model_name, model, gpu_id, train_loader, vis_train_loader, test_loader, criterion, optimizer, reg_method, prior_beta, reg_lambda, momentum_mu, blocks, hidden_dim, weightdecay, firstepochs, labelnum, max_epoch=25):
    logger = logging.getLogger('res_reg')
    res_regularizer_instance = ResRegularizer(prior_beta=prior_beta, reg_lambda=reg_lambda, momentum_mu=momentum_mu, blocks=blocks, feature_dim=hidden_dim, model_name=model_name)
    # hyper parameters
    print('Beginning Training')
    ...
    for weightdecay in weightdecay_list:
        for reg_lambda in reglambda_list:
            for prior_beta in priorbeta_list:
                ...
                if "MNIST" not in args.traindatadir: 
                    train_validate_test_resmlp_model(args.modelname, model_ft, gpu_id, train_loader, vis_train_loader, test_loader, criterion, optimizer_ft, args.regmethod, prior_beta, reg_lambda, momentum_mu, args.blocks, dim_vec[1], weightdecay, args.firstepochs, label_num, max_epoch=args.maxepoch)
                else:
                    train_validate_test_resmlp_model_MNIST(args.modelname, model_ft, gpu_id, train_loader, vis_train_loader, test_loader, criterion, optimizer_ft, args.regmethod, prior_beta, reg_lambda, momentum_mu, args.blocks, dim_vec[1], weightdecay, args.firstepochs, label_num, max_epoch=args.maxepoch)
